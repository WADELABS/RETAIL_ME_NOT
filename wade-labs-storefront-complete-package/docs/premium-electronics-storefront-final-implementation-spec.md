...
**Louisiana status:** LDR registration processed; resale certificate pending issuance and validation
...
Do not set `APPROVED_ACTIVE` until the issued certificate is available in LaTAP, downloaded, and validated.

The full Louisiana tax number and LaTAP username are sensitive operational data. Store them encrypted server-side and never expose them to the browser, source control, analytics, logs, screenshots, or public documentation.

## 25.2 Pending-State Rules

- Continue building and testing.
- Import only authorized supplier catalogs.
- Keep exemption-dependent distributor listings in `DRAFT`, `COMING_SOON`, or `PAUSED_COMPLIANCE`.
- Do not represent supplier purchases as tax-exempt.
- Do not use the registration letter as the resale certificate.
- Owned inventory may proceed only after tax, Stripe, policy, inventory, risk, profit, and fulfillment gates pass.
- Distributor inventory may proceed before exemption acceptance only under an explicit tax-paid procurement policy that includes supplier tax in all-in cost and still passes every profit floor.

## 25.3 Activation Workflow

1. Check the sales-tax account in LaTAP.
2. Check `View Letters`.
3. Check `Re-print Resale Certificate`.
4. Download the certificate when issued.
5. Store it in encrypted business storage.
6. Record exact approval and expiration dates.
7. Validate the certificate.
8. Set status to `APPROVED_ACTIVE`.
9. Submit it to each distributor.
10. Track distributor acceptance independently.
11. Activate only eligible listings.

## 25.4 Required Record

```ts
interface LouisianaComplianceRecord {
  jurisdiction: 'LA';
  ldrRegistrationStatus: 'NOT_STARTED' | 'SUBMITTED' | 'PROCESSED';
  ...
  encryptedLdrTaxNumber?: string;
  encryptedLdrAccountNumber?: string;
  encryptedLocationId?: string;
  encryptedLatapUsername?: string;
...
`APPROVED_ACTIVE` requires approval date, expiration date, and certificate document reference.

## 25.5 Distributor Acceptance and Use

Certificate issuance does not prove distributor acceptance. Store submission, acceptance, rejection, exemption reference, applicable jurisdictions, account status, and dropship status separately.

The certificate may be used only for qualifying resale purchases. Business-use withdrawals must be reclassified and routed to sales/use-tax accounting.
---
...
63. Louisiana registration state is `PROCESSED`.
64. Louisiana resale certificate remains `PENDING` until issued, downloaded, and validated.
65. Pending state blocks exemption-dependent distributor activation by default.
66. `APPROVED_ACTIVE` requires exact dates and a certificate document reference.
67. Distributor acceptance is tracked independently from certificate issuance.
68. Business-use withdrawal triggers use-tax accounting.
69. Supplier onboarding records certificate submission and acceptance independently.
...
- Louisiana registration processed and resale-certificate pending-state controls
- Stripe PaymentIntents
...
# 36. Advertising, Marketing, Growth, Scaling, and UI/UX Authority
...
No campaign or interface experiment may scale unless it increases contribution and passes fulfillment, cash, risk, support, inventory, compliance, accessibility, and performance guardrails.
